...
--           (Γ ∣ ((qAny qR∘ R) R+ S) R+ T ⊢ (Cons eKey eVal eDict) ∣ (τDict τKey τVal)) 
  ...
  TyJoin : {n : ℕ} {Γ : Vec τ n} {R₁ : Vec q n} {R₂ : Vec q n} {e₁ e₂ : e} {τ τ₀ : τ} {p : IsSemilat τ τ₀} →
           (Γ ∣ R₁ ⊢ e₁ ∣ τ) → (Γ ∣ R₂ ⊢ e₂ ∣ τ) → (Γ ∣ R₁ R+ R₂ ⊢ (Join τ e₁ e₂) ∣ τ)  
          
  TyAbs : {n : ℕ} {Γ : Vec τ n} {R : Vec q n} {q : q} {body : e} {σ τ : τ} → 
          ((σ ∷ Γ) ∣ (q ∷ R) ⊢ body ∣ τ) → (Γ ∣ R ⊢ (Abs body) ∣ (τFun σ q τ))    

  TyPair : {n : ℕ} {Γ : Vec τ n} {R₁ : Vec q n} {R₂ : Vec q n} {e₁ e₂ : e} {τ₁ τ₂ : τ} →
           (Γ ∣ R₁ ⊢ e₁ ∣ τ₁) → (Γ ∣ R₂ ⊢ e₂ ∣ τ₂) → (Γ ∣ R₁ R+ R₂ ⊢ (Pair e₁ e₂) ∣ (τProduct τ₁ τ₂))
  
  TyProj1 : {n : ℕ} {Γ : Vec τ n} {R : Vec q n} {pair : e} {τ₁ τ₂ : τ} → 
            (Γ ∣ R ⊢ pair ∣ (τProduct τ₁ τ₂)) → (Γ ∣ R ⊢ (Proj1 pair) ∣ τ₁)

  TyProj2 : {n : ℕ} {Γ : Vec τ n} {R : Vec q n} {pair : e} {τ₁ τ₂ : τ} → 
            (Γ ∣ R ⊢ pair ∣ (τProduct τ₁ τ₂)) → (Γ ∣ R ⊢ (Proj1 pair) ∣ τ₁)

  TyInl : {n : ℕ} {Γ : Vec τ n} {R : Vec q n} {body : e} {τL τR : τ} →
          (Γ ∣ R ⊢ body ∣ τL) → (Γ ∣ R ⊢ Inl τL τR body ∣ τSum τL τR)

  TyInr : {n : ℕ} {Γ : Vec τ n} {R : Vec q n} {body : e} {τL τR : τ} →
          (Γ ∣ R ⊢ body ∣ τR) → (Γ ∣ R ⊢ Inr τL τR body ∣ τSum τL τR)

  TyCase : {n : ℕ} {Γ : Vec τ n} {R₀ R₁ R₂ : Vec q n} {e₀ e₁ e₂ : e} {τ₁ τ₂ τRes : τ} {q₁ q₂ : q} → 
           (Γ ∣ R₀ ⊢ e₀ ∣ τSum τ₁ τ₂) → ((τ₁ ∷ Γ) ∣ (q₁ ∷ R₁) ⊢ e₁ ∣ τRes) → ((τ₂ ∷ Γ) ∣ (q₂ ∷ R₂) ⊢ e₂ ∣ τRes) →
