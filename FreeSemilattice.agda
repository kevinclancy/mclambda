...
∨-lemma : {l1 l2 l3 : List Carrier} → {a : Carrier} → {f1 : IsFreeList _<_ _⊑_ l1} → {f2 : IsFreeList _<_ _⊑_ l2} → 
           ...
           (a ∈ l1 × ¬ Any (a ⊑_) l2) ⊎ (a ∈ l2 × ¬ Any (a ⊑_) l1)
...
∨-lemma {l1@(h1 ∷ t1)} {l2@(h2 ∷ t2)} {_} {a} {f1@(∷-Free _ _ _ _ ft1)} {f2@(∷-Free _ _ min2 incomp2 _)} {f3} PE.refl a∈l3 | l⊑r h1⊑h2 ¬h2⊑h1 = 
  let
    p : (a ∈ t1 × ¬ Any (a ⊑_) l2) ⊎ (a ∈ l2 × ¬ Any (a ⊑_) t1)
    p = ∨-lemma {f1 = ft1} {f2 = f2} {f3 = f3} PE.refl a∈l3
  in
    [_,_] c1 c2 p
  where
    c1 : (a ∈ t1 × ¬ Any (a ⊑_) l2) → (a ∈ l1 × ¬ Any (a ⊑_) l2) ⊎ (a ∈ l2 × ¬ Any (a ⊑_) l1)
    c1 p1 = inj₁ $ (there (proj₁ p1) , proj₂ p1)
    ...
    c2 : (a ∈ l2 × ¬ Any (a ⊑_) t1) → (a ∈ l1 × ¬ Any (a ⊑_) l2) ⊎ (a ∈ l2 × ¬ Any (a ⊑_) l1)
    c2 p2 = inj₂ $ (proj₁ p2 , ¬a⊑l1 (proj₁ p2) (proj₂ p2))
∨-lemma {l1@(h1 ∷ t1)} {l2@(h2 ∷ t2)} {_} {a} {f1@(∷-Free _ _ _ incomp1 _)} {f2@(∷-Free _ _ _ _ ft2)} {f3} PE.refl a∈l3 | r⊑l ¬h1⊑h2 h2⊑h1 = 
  let 
    p : (a ∈ l1 × ¬ Any (a ⊑_) t2) ⊎ (a ∈ t2 × ¬ Any (a ⊑_) l1)
  ...
  where
    ...
    c1 : (a ∈ l1 × ¬ Any (a ⊑_) t2) → (a ∈ l1 × ¬ Any (a ⊑_) l2) ⊎ (a ∈ l2 × ¬ Any (a ⊑_) l1)
    c1 p1 = inj₁ $ ( proj₁ p1 , ¬a⊑l2 (proj₁ p1) (proj₂ p1) )

    c2 : (a ∈ t2 × ¬ Any (a ⊑_) l1) → (a ∈ l1 × ¬ Any (a ⊑_) l2) ⊎ (a ∈ l2 × ¬ Any (a ⊑_) l1)
    c2 p2 = inj₂ $ (there (proj₁ p2) , proj₂ p2)

∨-lemma {h1 ∷ t1} {h2 ∷ t2} {_} {a} {f1} {f2} {f3} PE.refl a∈l3 | l≡r h1≡h2 = {!!}
∨-lemma {h1 ∷ t1} {h2 ∷ t2} {_} {a} {f1} {f2} {f3} PE.refl a∈l3 | l∥r h1∥h2 = {!!}
